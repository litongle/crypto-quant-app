...
class AuditLog(Base):
    ...
    user_id: Mapped[int] = mapped_column(
        ForeignKey("users.id"), index=True, comment="操作用户"
    )

    # 操作分类
    action: Mapped[str] = mapped_column(
        String(50), index=True, comment="操作类型: order_create/order_submit/order_cancel/param_update/stop_loss_set/take_profit_set/account_update/strategy_start/strategy_stop"
    )
    resource: Mapped[str] = mapped_column(
        String(50), nullable=True, comment="资源类型: order/strategy/account/position"
    )
    resource_id: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="资源ID"
    )
    ...
    old_value: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="变更前数据快照"
    )
    new_value: Mapped[dict | None] = mapped_column(
        JSON, nullable=True, comment="变更后数据快照"
    )
