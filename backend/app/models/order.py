...
class Order(Base):
    ...
    commission: Mapped[Decimal] = mapped_column(Numeric(20, 8), default=0, comment="手续费")
    ...
    signal_id: Mapped[int | None] = mapped_column(
        ForeignKey("signals.id", use_alter=True, name="fk_order_signal_id"),
        nullable=True,
        index=True,
    )
    ...
    # 关系
    account: Mapped["ExchangeAccount"] = relationship("ExchangeAccount", back_populates="orders")
...
class Signal(Base):
    ...
    # 信号动作
    action: Mapped[str] = mapped_column(Enum("buy", "sell", "close", name="signal_action"))

    # 信号详情
    confidence: Mapped[Decimal] = mapped_column(Numeric(5, 4), comment="置信度 0.0000 - 1.0000")
    ...
    market_data: Mapped[dict | None] = mapped_column(JSON, nullable=True, comment="市场数据快照")
