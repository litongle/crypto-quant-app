...
| ADR-013 | 单用户化 + VPS 化（撤销 ADR-008 的安装向导/多用户/审计部分） | ✅ |
...
**背景**：项目定位收敛为「个人自用 + 给朋友源码自部署」（不再是面向多用户的 SaaS）。
ADR-008 引入的安装向导、多用户体系、TOTP 2FA、审计中间件在自用场景是纯负担——
个人合约系统的核心矛盾是 7×24 稳定运行 + 风控 + 告警，不是用户管理。

**决策**：
1. **认证收敛单用户**：保留 JWT 登录，但只支持唯一 admin；用户名 + bcrypt 哈希
   存 `.env`；启动时 `seed_admin()` 同步到 `users` 表（user_id 固定为 1）
2. **删除安装向导**：`/api/v1/setup/*` 和 `/web/setup` 全部移除；用 `.env.example`
   + `scripts/generate_admin_hash.py` 替代
3. **删除 TOTP/2FA**：撤销 ADR-006 中 TOTP 部分；`totp_service.py`、User 模型
   `totp_*` 字段、相关 API 端点全删
4. **删除审计中间件 + audit_logs 表**：撤销 ADR-006 中审计部分；事件流改为
   只聚合 Signal + 策略自停（不再依赖 AuditLog）
5. **强制 PostgreSQL**：撤销 ADR-008 的「SQLite 默认零配置」；docker-compose
   内置 PG，默认 `DATABASE_URL` 指向容器内 PG
6. **VPS 化部署**：新增 `docs/DEPLOY-VPS.md` + `deploy/Caddyfile.example`，
   Caddy v2 自动 HTTPS

**备选**：
- 完全去掉应用层登录靠反向代理 BasicAuth：合约场景风险太高，否决
- 保留 TOTP：自用价值与维护负担不匹配，否决
- 分阶段渐进重构：service/route 耦合强，分阶段会留长时间中间炸状态，否决

**正面**：
- 后端代码 -1173 行（净删除约 7 个文件 + 5 个文件大幅瘦身）
- VPS 单用户部署 30 分钟到 HTTPS 上线
- 启动日志干净，无 SaaS 残留噪声

**负面**：
- 事件流降级：不再有「订单提交/风险告警/错误」类事件（这些原本由审计聚合）。
  将来如需回补，建议由 strategy_runner 主动写入 `strategy_events` 表，避免
  再次引入「自己审计自己」的中间件反模式
- 老库已存多用户的，迁移后只保留第一条；其余记录保留但需手动清理

**日期/决策者**：2026-05-13 | @owner
...
---
*最后更新：2026-05-13（ADR-013 单用户化 + VPS 化）*
